...
class ValidationIssue:
	...
	def __str__(self) -> str:
		...
		field_info = f"[{_(self.field)}] " if self.field else ""
		...
		for row in template.rows:
			...
			if row.data_source in ["Account Data", "Calculated Amount", "Custom API"]:
				if not row.calculation_formula:
					result.add_error(
						ValidationIssue(
							message=_("Formula is required for {0}").format(_(row.data_source)),
							row_idx=row.idx,
						)
					)
	...
	def _validate_missing_dependencies(self) -> ValidationResult:
		...
		for ref_code, deps in self.dependencies.items():
			...
			if undefined:
				...
				result.add_error(
					ValidationIssue(
						message=_("Line References undefined in Formula: {0}").format(
							", ".join(undefined)
						),
						row_idx=row_idx,
					)
	...
	def _test_formula_evaluation(self, formula: str, available_codes: list[str]) -> str | None:
		try:
			...
			if not isinstance(result, (int, float)):  # noqa: UP038
				return _("Formula must return a numeric value, got {0}").format(
