...
class TestZhFinanceImage(TestCase):
	...
	def test_image_compiles_translations_and_frontend_assets(self):
		...
		self.assertIn("label: __(value)", self.setup_wizard_source)
		self.assertIn('let standard_options = ["Standard", "Standard with Numbers"]', self.setup_wizard_source)
		self.assertIn("silent: true", self.setup_wizard_source)
		self.assertIn("erpnext.setup.set_chart_options(slide, r.message)", self.setup_wizard_source)
		self.assertIn("erpnext.setup.set_chart_options(slide, standard_options)", self.setup_wizard_source)
	...
	def test_report_datatable_controls_are_localized(self):
		...
		self.assertIn('const modal_title_selector = ".modal-title";', self.browser_overrides)
